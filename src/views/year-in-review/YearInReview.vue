  ...
  albumYears.forEach((year, y) => {
    year.forEach((entry, a) => {
      albumArt(entry.artist, { album: entry.album })
        .then((url: string) => { albumYearUrls.value[y][a] = url; })
...
<template>
  ...
  <div class="flex items-center justify-center min-h-screen py-24 px-8">
    <div class="grid grid-cols-2 gap-6 w-full max-w-3xl">
      <div v-for="(year, index) in albumYears" :key="index" class="year-card cursor-pointer rounded-2xl overflow-hidden"
        ...
        <div class="grid grid-cols-2">
          <div v-for="(entry, corner) in year" :key="corner" class="relative aspect-square overflow-hidden">
            ...
            <!-- Album art -->
            <img v-if="albumYearUrls[index][corner] !== null" :src="albumYearUrls[index][corner] ?? ''"
              :alt="entry.album" class="w-full h-full object-cover transition-opacity duration-500"
